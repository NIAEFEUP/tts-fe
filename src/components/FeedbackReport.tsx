  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <Button variant="icon" className="flex flex-row gap-x-2 rounded-full">
          <FlagIcon className="h-5 w-5 text-black dark:text-white md:dark:text-black" />
          <p className="hidden text-black sm:block dark:text-white md:dark:text-black">Feedback</p>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="ml-5">
        <Tabs defaultValue="suggestion">
          <TabsList className="w-full">
            <TabsTrigger value="suggestion" onClick={() => setReportType(ReportType.Suggestion)}>
              Sugestão
            </TabsTrigger>
            <TabsTrigger value="bug" onClick={() => setReportType(ReportType.Bug)}>
              Bug
            </TabsTrigger>
          </TabsList>
        </Tabs>
