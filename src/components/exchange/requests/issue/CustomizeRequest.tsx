...
export const CustomizeRequest = ({
  ...
  const { relatedExchanges, loading } = useRelatedExchanges(
    previewingForm + [requests.entries()].map(([k, v]) => `${k}=${v}`).join(','),
    requests
  )
  ...
  return (
    <div className="flex flex-col gap-y-4">
      {selectedCourseUnits.length === 0 ? (
        ...
        <>
          <div className="mt-4 flex w-full flex-row justify-between gap-x-2">
            <div className="flex w-full flex-row gap-x-2">
              {selectedCourseUnits.length > 0 && (
                <>
                  <Button
                    className={`${
                      selectedCourseUnits.length > 0 ? 'w-1/2' : 'w-full'
                    } hover:bg-gray-150 bg-gray-200 text-gray-800`}
                    onClick={() => {
                      setExchangeSidebarStatus(ExchangeSidebarStatus.SHOWING_REQUESTS)
                    }}
                  >
          ...
          <div className="flex flex-row gap-2">
            ...
            />
            <label htmlFor="person-to-exchange">Tenho pessoas com quem trocar</label>
          </div>
