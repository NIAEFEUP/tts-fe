  ...
  return (
    ...
    >
      ...
      <DialogContent className="flex flex-col">
        <DialogHeader>
          <DialogTitle className="mb-4 text-center">Prever visualização do pedido</DialogTitle>
          {hasSomeConflict && (
            <Alert type={conflictSeverity ? AlertType.error : AlertType.warning}>
              <p>
                {conflictSeverity ? (
                  ...
                  <>
                    Colisões com <strong>aulas teóricas e práticas</strong> só devem ser submetidas se forem inevitáveis
                    ou se for possível assistir à aula teórica noutro turno.
                  </>
                )}
              </p>
            </Alert>
        ...
        {currentView === CurrentView.OTHER_REQUESTS && (
          <RelatedExchanges
            setUrgentMessage={setSendUrgentMessage}
            setCurrentView={setCurrentView}
            requests={relatedExchanges}
          />
        )}

        {currentView === CurrentView.CONFIRMATION && (
          <ExchangeSubmissionConfirmation
            requests={requests}
