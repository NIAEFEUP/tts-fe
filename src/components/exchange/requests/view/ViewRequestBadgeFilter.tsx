    ...
    <div className="flex flex-row items-center gap-x-2">
      <Badge
        className={`${filterCourseUnits.has(courseUnit.id) ? 'bg-primary text-white' : 'bg-gray-200 text-gray-700'} 
                    hover:bg-primary flex cursor-pointer flex-row items-center gap-x-1 hover:text-white`}
        onClick={() => {
      ...
      <DropdownMenu>
        <DropdownMenuTrigger className="p-0 xl:p-0" asChild>
          <Button variant="icon" className="h-5 w-5 p-0 text-black dark:text-white">
            <ChevronDownIcon className="h-5 w-5" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent className="flex max-h-72 flex-col gap-y-2 overflow-y-scroll">
