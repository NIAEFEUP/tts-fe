...
const RequestCardSkeletons = () => {
  const skeletons = Array.from({ length: 3 }, (_, i) => (
    <div className="flex w-full flex-row items-center space-x-4" key={`view-request-skeleton-${i}`}>
    ...
    <div className="relative flex flex-row flex-wrap items-center justify-center gap-x-2 gap-y-2 lg:justify-start">
      ...
      <Tabs defaultValue="todos" className="mt-2 w-full">
        <TabsList className="flex flex-row justify-between dark:text-white">
          <TabsTrigger onClick={() => setCurrentRequestTypeFilter(0)} value="todos">
            Todos
          </TabsTrigger>
          <TabsTrigger onClick={() => setCurrentRequestTypeFilter(1)} value="meus-pedidos">
            Enviados
          </TabsTrigger>
          <TabsTrigger onClick={() => setCurrentRequestTypeFilter(2)} value="recebidos">
            Recebidos
          </TabsTrigger>
        </TabsList>
        <TabsContent value="todos" className="mt-4">
          ...
          />
          <div ref={requestCardsContainerRef} className="mt-4 flex max-h-[70vh] flex-col gap-y-3 overflow-y-auto">
            {isLoading ? (
        ...
        <TabsContent value="meus-pedidos" className="mt-4">
          ...
          />
          {/* <ViewSentRequests /> */}
          <div className="mt-4 flex max-h-screen flex-col gap-y-3 overflow-y-auto">
            {isLoading ? (
              <RequestCardSkeletons />
            ) : (
              <EmptyRequestGuard requests={requests}>
