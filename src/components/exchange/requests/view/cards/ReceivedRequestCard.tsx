...
export const ReceivedRequestCard = ({ request }: Props) => {
  const {
    open,
    setOpen,
    selectedOptions,
        ...
        >
          ...
          <CardContent className="p-0 px-4">
            {open && (
              <>
                {request.options
                  .filter((option) => option.participant_nmec === user?.username)
                  .map((option) => (
                    <ListRequestChanges
                      ...
                      userWillExchangeTo={
                        request.options.find(
                          (o: DirectExchangeParticipant) =>
                            o.class_participant_goes_from.id === option.class_participant_goes_to.id,
                        )?.participant_name
                      }
                    />
                  ))}
          ...
          <CardFooter className={open ? '' : 'hidden'}>
            <div className="flex flex-row justify-between w-full items-center">
              <form className="flex flex-row gap-2">
                {!request.accepted &&
                  requestStatus !== StudentRequestCardStatus.CANCELED &&
                  request.pending_motive === DirectExchangePendingMotive.USER_DID_NOT_ACCEPT && (
                    ...
                    >
                      {isAcceptingDirectExchange ? <MoonLoader size={20} /> : <span>Aceitar</span>}
                    </Button>
                  )}
              </form>
            </div>
