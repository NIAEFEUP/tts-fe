...
export const MineRequestCard = ({ request, mutate }: Props) => {
  const { open, setOpen, selectedOptions, setSelectedOptions, setSelectAll, togglePreview, setRequestStatus } =
  ...
  const { trigger: cancelMarketplaceExchange, isMutating: cancelingMarketplaceExchange } = useCancelMarketplaceExchange(
    request.id
  )
  ...
  const [draftMessage, setDraftMessage] = useState<string | undefined>(() =>
    request.type === 'urgentexchange' ? (request as UrgentRequest).message : undefined
  )
  ...
  return (
    ...
    >
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    <Button
                      disabled={saving}
                      onClick={() => {
                        handleSave(request.id, draftMessage ?? '')
                      }}
                    >
                      Salvar
                    </Button>
                    <Button
                      ...
                      onClick={() => {
                        setEditing(false)
                        setDraftMessage((request as UrgentRequest).message)
                      }}
                    >
                      Cancelar
                    </Button>
      ...
      <CardFooter className={open ? '' : 'hidden'}>
        {!(request as MarketplaceRequest).canceled && !request.accepted && request.type != 'urgentexchange' && (
          <Button
            variant="destructive"
            onClick={async () => {
          ...
          >
            {cancelingMarketplaceExchange ? <MoonLoader size={20} /> : 'Cancelar'}
          </Button>
        )}
      </CardFooter>
    </Card>
