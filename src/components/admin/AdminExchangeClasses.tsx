      ...
      {!loading && classes && classes.length > 0 && (
        <>
          <div className="mb-6 space-y-4">
            <Tabs value={selectedCourseId} onValueChange={setSelectedCourseId} className="w-full">
              <TabsList className="flex h-auto w-full p-1">
                {courses.map((course) => (
                  <TabsTrigger
                    key={course.id}
                    value={course.id}
                    className="data-[state=active]:bg-primary flex-grow data-[state=active]:text-white"
            ...
            <div className="w-full">
              <div className="mb-1.5 text-sm font-medium">Filtrar por cadeira:</div>
