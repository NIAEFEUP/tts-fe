 ...
 */
export const AdminPreviewSchedule = ({ originalSchedule, classesToAdd }: Props) => {
  ...
  useEffect(() => {
    ...
    setSchedule([
      ...prevSchedule,
      ...classesToAdd.filter((classDescriptor) => classDescriptor.classInfo && classDescriptor.courseInfo),
    ])
  }, [originalSchedule])
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-h-fit max-w-fit p-4">
        <DialogHeader className="w-[50em]">
          <DialogTitle>Prever horário</DialogTitle>
        </DialogHeader>
        <ScheduleContext.Provider
          value={{
            originalExchangeSchedule: originalSchedule?.filter(
              (classDescriptor) => classDescriptor.classInfo && classDescriptor.courseInfo
            ),
