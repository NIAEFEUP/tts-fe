    ...
    <div className="flex flex-row gap-x-2">
      ...
      >
        ...
        <SelectContent>
          ...
          {courses?.map((course) => (
            <SelectItem key={`course-select-item-${course.id}`} value={`${course.id}`}>
              {course.acronym}
            </SelectItem>
          ))}
        </SelectContent>
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="outline" className="flex w-full flex-row gap-x-2">
            <p>
              Estado
                ...
                {activeStatesPossibleValues.map((state) => (
                  ...
                  >
                    <div className="flex flex-row gap-x-2">
                      ...
                      {activeStates.includes(state) && <Check className="h-5 w-5" />}
                    </div>
                  </CommandItem>
                ))}
              </CommandGroup>
