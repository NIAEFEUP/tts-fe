...
const ShareButtons = ({ majorHook, schedule, multipleOptionsHook, setIsImportedSchedule }: Props) => {
        ...
        if (i !== multipleOptions.index) {
          ...
          for (let y = 0; y < all_options[i].length; y++) {
            for (let x = 0; x < placeholder_course_options.length; x++) {
              if (
                placeholder_course_options[x].course.info.course_unit_id ===
                all_options[i][y].course.info.course_unit_id
              ) {
    ...
    for (let i = 0; i < all_options.length; i++) {
      if (i !== multipleOptions.index) {
        for (let x = 0; x < placeholder_course_options.length; x++) {
          ...
          for (let y = 0; y < all_options[i].length; y++) {
            if (
              placeholder_course_options[x].course.info.course_unit_id === all_options[i][y].course.info.course_unit_id
            ) {
              found = true
              break
            }
  ...
  const importSingleSchedule = async (scheduleString: string) => {
    ...
    for (let i = 0; i < courses_info.length; i++) {
      ...
      if (course_already_exists) continue
    ...
    <>
      {showDiv ? (
        ...
        >
          ...
          <button
            ...
            className="-mx-1.5 -my-1.5 ml-auto inline-flex  h-8 w-8 items-center rounded-lg bg-white p-1.5 text-gray-400 
                                                hover:bg-gray-100 hover:text-gray-900 focus:ring-2 focus:ring-gray-300 dark:bg-gray-800 dark:text-gray-500 dark:hover:bg-gray-700 
      ...
      <div className="mt-1 grid w-full grid-flow-row grid-cols-7 grid-rows-1 gap-2">
        <div className="relative col-span-6">
          ...
          <input
            id="schedule-input"
            placeholder="Insere o link do horário..."
            className="inline-flex w-full items-center justify-center whitespace-nowrap rounded border-2
                        border-gray-300 bg-white p-2.5 text-center text-sm font-normal text-black outline-none transition hover:opacity-80 focus:border-tertiary 
                        disabled:cursor-not-allowed disabled:opacity-50 dark:border-tertiary dark:bg-darkish dark:text-white dark:placeholder:text-white dark:focus:border-gray-300"
            required
            onKeyDown={(event) => {
              if (event.key === 'Enter') {
                openDecisionModal()
