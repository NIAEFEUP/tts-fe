...
const Refresh = () => {
  ...
  const updateCourses = async () => {
    ...
    if (mismatchedMap.size > 0) {
      const coursesToUpdate = pickedCourses.filter((course) => mismatchedMap.has(course.id))
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="group relative">
            <Button variant="icon" className="relative bg-emerald-800" onClick={updateCourses} disabled={isLoading}>
              <span className="absolute -right-1 -top-1 block h-3 w-3 rounded-full border border-white bg-red-600"></span>
