...
const OptionsController = () => {
  ...
  return (
    <ReactSortable
      className="m-y-2 flex w-full flex-row justify-center gap-2 overflow-x-auto py-2 text-center lg:justify-start"
    ...
    <TooltipProvider delayDuration={300}>
      <Tooltip>
        <TooltipTrigger
          ...
          className={`
            w-15 hover:dark:border-primary/50 group relative box-border flex aspect-square h-10 cursor-pointer
            flex-col items-center justify-center rounded-md border-2 border-transparent
            p-2 dark:shadow
            ${
        ...
        >
          ...
          <img
            src={option.icon}
            className="h-9 w-9 transform duration-200 ease-in-out group-hover:mt-3"
            alt={option.name}
          />
        </TooltipTrigger>
